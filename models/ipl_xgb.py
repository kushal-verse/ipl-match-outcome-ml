...
def train(preprocessed_csv):
    ...
    logloss      = log_loss(y_test_eval, y_pred_proba)
    conf_matrix  = confusion_matrix(y_test_eval, y_pred)
    class_report = classification_report(y_test_eval, y_pred, target_names=['Loss', 'Win'])
    ...
    ax3.bar(x_pos - width / 2, [(y_test_eval == 0).sum(), (y_test_eval == 1).sum()],
